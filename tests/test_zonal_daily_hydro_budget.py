"""Daily hydro-budget coverage for the zonal transportation model."""
...
from sdom import OptimizationResults, get_default_solver_config_dict, initialize_model, load_data, run_solver
...
def test_zonal_daily_hydro_budget_builds_and_solves(tmp_path):
    """Zonal daily hydro budgets build per-area bounds and solve through public APIs."""
    fixture_dir = _daily_budget_fixture(tmp_path)
    data = load_data(str(fixture_dir))
    ...
    results = run_solver(model, _highs_config(), case_name="zonal_daily_hydro")
...
def test_hydro_budget_csv_check_supports_literal_and_tagged_columns(
    ...
    """CSV matching accepts the legacy column and sums zonal tagged columns."""
